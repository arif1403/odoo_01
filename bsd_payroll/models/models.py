...
class BSDPosting(models.Model):
    ...
    def action_posting_gaji(self):
        ...
        for norek, net_wage in net_wage_by_employee.items():
            ...
            response_2 = requests.post(endpoint_setoran_coa, data=json_data_, headers=headers_nasabah, timeout=30)
        ...
        if os.path.exists(excel_file_path):
            ...
            # Log the attachment ID and URL
            # print(f"Attachment ID: {attachment.id}")
            # print(f"Attachment URL: /web/content/{attachment.id}?download=true")
    ...
    def action_post_weekly(self):
        ...
        # username = "sinthadaya_oddo"
        # password = "sinthadayaoddof@Gfk$7W"
        client_secret = "94d5025d57778ae87147f302627adbe5d2a49dec"
    ...
    def action_post_transport(self):
        ...
        try:
            ...
            if os.path.exists(excel_file_path):
                ...
                # Log untuk memastikan bahwa attachment dibuat
                ...
                file_url = f"/web/content/{attachment.id}?download=true"
                file_url_with_mime = f"{file_url}&mimetype=application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                print(f"File URL: {file_url_with_mime}")

                return {
                    'type': 'ir.actions.act_url',
                    'name': 'Download Excel File',
                    'url': file_url_with_mime,
                    'target': 'self',  # Buka di tab baru
                }
        except requests.exceptions.ConnectTimeout as e:
            print("Koneksi time out:", e)
    ...
    def action_post_insentif(self):
        ...
        # password = "sinthadayaoddof@Gfk$7W"
        client_secret = "94d5025d57778ae87147f302627adbe5d2a49dec"
        api = "https://testing.msodc.co.id/los/api"
        path = [
                "/auth",
                "/tabungan/info",
                "/trans/setoran/coa",
        ...
        delay_seconds = 10
        excel_file_path = "report_insentif.xlsx"
        workbook = xlsxwriter.Workbook(excel_file_path)
        worksheet = workbook.add_worksheet()
        ...
        try:
            for norek, net_wage in insentif.items():
                ...
                worksheet.write(row_counter, 5, response_json["rekening"]["name"])
                worksheet.write(row_counter, 6, response_json["sumber"]["norek"])
            print("Posting Insentif success!!")
            workbook.close()
    ...
    def action_post_thr(self):
        ...
        delay_seconds = 10
        excel_file_path = "report_thr.xlsx"
        workbook = xlsxwriter.Workbook(excel_file_path)
        worksheet = workbook.add_worksheet()
    ...
    def action_angsuran(self):
        ...
        header = ["Code", "Message","TransID","No_Kredit","Total"]
        for col, title in enumerate(header):
            worksheet.write(0, col, title)
        # Counter untuk menentukan baris pertama
        row_counter = 0
    ...
    def action_nonkas(self):
        ...
        # print(hmac.new(client_secret.encode(), payload, hashlib.sha256).hexdigest())
        # Header untuk permintaan
        headers = {
            "x-signature": signature,
            "Content-Type": "application/json"
